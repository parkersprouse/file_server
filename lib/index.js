...
export function toBreadcrumbs(path, query) {
  ...
  const parts = without(cleaned, 'f').slice(0, -1);
  return [
      // don't add if current route is root
      { label: 'Root', url: `/${query ? `?${query}` : ''}` },
      ...parts.map((label, index) => {
        const url = `/f/${parts.slice(0, index + 1).join('/')}${query ? `?${query}` : null}`;
        return { label, url };
      }),
  ];
}
