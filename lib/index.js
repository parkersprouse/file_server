...
import { cloneDeep, orderBy, without } from 'lodash-es';
import qs from 'qs';
...
export function toBreadcrumb(path) {
  const route_halves = path.split('?');
  const cleaned = route_halves[0].split('/').filter((part) => !!part);
  const parts = without(cleaned, 'f');
  return parts.map((label, index) => {
    let url = `/f/${parts.slice(0, index + 1).join('/')}`;
    if (route_halves[1]) url += `?${route_halves[1]}`;
    return {
      label,
      url,
    };
