...
function executeProbe(input) {
  const probe_parameters = ['-v', 'error', '-show_format', '-show_streams'];
  if (typeof input === 'string') return execa(ffprobe.path, [...probe_parameters, input]);
...
export function formatDuration(full_seconds) {
  return new Date(1000 * full_seconds).toISOString().slice(11, 19);
}

export function formatLastUpdated(epoch) {
  const date = new Date(epoch);
  const date_string = date.toLocaleString('en-US', { day: 'numeric', month: 'short', year: 'numeric' });
  const time_string = date.toLocaleString('en-US', { hour: '2-digit', hour12: true, minute: '2-digit' });
  return `${date_string}, \n${time_string}`;
}
